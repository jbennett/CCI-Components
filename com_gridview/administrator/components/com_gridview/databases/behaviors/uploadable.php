...
class KDatabaseBehaviorUploadable extends KDatabaseBehaviorAbstract {
	...
	protected function _beforeTableUpdate(KCommandContext $context)
	{
		...
		if ($file['error'] !== 0 && $file['error'] !== 4) {
			JError::raiseWarning('300', 'Error uploading image: '.$file['error']);
			return false;
		}
	...
	protected function thumbName($filename, $thumb) {
		$name  = (isset($thumb['prefix']))? "{$thumb['prefix']}": "";
		$name .= "{$filename}";
		$name .= (isset($thumb['suffix']))? "{$thumb['suffix']}" : "";
		return $name;
	}
	...
	protected function createThumb($filename, $extension, $thumbinfo, &$original)
	{
		...
		$ratio	= $width/$height;
		$bw		= isset($thumbinfo->greyscale)? $thumbinfo->greyscale : false;
		...
		if ($bw) {
			imagefilter($final_image, IMG_FILTER_GRAYSCALE);
		}
